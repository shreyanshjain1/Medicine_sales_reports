  ...
  if (!$errors) {
    ...
    if (!$stmt) {
      ...
      if ($stmt->execute()) {
        ...
        header('Location: ' . url('task_view.php?id=' . $id));
        exit;
      }
      $errors[] = 'Failed to update task.';
      $stmt->close();
...
<div class="card">
  ...
  <form class="form" method="post" action="<?= url('task_edit.php?id='.(int)$ev['id']) ?>">
    <?php csrf_input(); ?>
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:14px">
      ...
      <a class="btn" href="<?= url('task_view.php?id='.(int)$ev['id']) ?>">Cancel</a>
    </div>
  </form>
</div>
...
(function(){
  ...
  async function loadDoctors(city){
    ...
    if (!city) return;
    try{
      const r = await fetch('api_doctors.php?city=' + encodeURIComponent(city), {cache:'no-store'});
