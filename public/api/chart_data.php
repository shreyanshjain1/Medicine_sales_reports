...
if ($mine) {
  ...
  if (!$res) {
    api_json_error('Unable to load personal chart data.', 500, [$mysqli->error ?: 'Query failed']);
  }
  $labels=[]; $data=[];
  while($r=$res->fetch_assoc()){ $labels[]=$r['d']; $data[]=(int)$r['c']; }
  api_json_success(['byDate'=>['labels'=>$labels,'data'=>$data]], 'Personal chart data loaded.');
}

if (is_employee()) {
  api_json_error('Forbidden.', 403, ['Managers or district managers only.']);
}
...
if (!$res) api_json_error('Unable to load employee chart data.', 500, [$mysqli->error ?: 'Query failed']);
...
if (!$res) api_json_error('Unable to load timeline chart data.', 500, [$mysqli->error ?: 'Query failed']);
while($r=$res->fetch_assoc()){ $labels[]=$r['d']; $data[]=(int)$r['c']; }
...
if (!$res) api_json_error('Unable to load status chart data.', 500, [$mysqli->error ?: 'Query failed']);
while($r=$res->fetch_assoc()){ $labels[]=$r['status']; $data[]=(int)$r['c']; }
$byStatus=['labels'=>$labels,'data'=>$data];

api_json_success(['byEmployee'=>$byEmployee,'byDate'=>$byDate,'byStatus'=>$byStatus], 'Chart data loaded.');
