...
<div class="grid two-panels masters-grid">
  ...
  <div class="card">
    ...
    <?php foreach($rows as $row): ?><tr><td><strong><?= e($row['name']) ?></strong></td><td><?= e($row['category']) ?></td><td class="muted"><?= e($row['notes']) ?></td><td><div class="table-actions"><a class="btn tiny" href="<?= route_url('masters/medicines_master.php', ['edit'=>(int)$row['id']]) ?>">Edit</a><form method="post" onsubmit="return confirm('Archive this medicine?');"><?php csrf_input(); ?><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= (int)$row['id'] ?>"><button class="btn tiny danger" type="submit">Archive</button></form></div></td></tr><?php endforeach; ?>
    </tbody></table></div>
  </div>
</div>
