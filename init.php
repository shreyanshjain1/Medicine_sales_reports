...
function summary_scope_where(string $alias='r'): string {
  return reports_scope_where($alias);
}

function summary_filter_sql(string $alias='r', array $filters=[]): string {
  global $mysqli;
  $a = preg_replace('/[^a-zA-Z0-9_]/','', $alias);
  $where = 'WHERE ' . summary_scope_where($a);
  $dateFrom = trim((string)($filters['date_from'] ?? ''));
  $dateTo = trim((string)($filters['date_to'] ?? ''));
  $status = trim((string)($filters['status'] ?? 'all'));
  $employeeId = (int)($filters['employee_id'] ?? 0);
  if ($dateFrom !== '') $where .= " AND DATE({$a}.visit_datetime) >= '" . $mysqli->real_escape_string($dateFrom) . "'";
  if ($dateTo !== '') $where .= " AND DATE({$a}.visit_datetime) <= '" . $mysqli->real_escape_string($dateTo) . "'";
  if ($employeeId > 0) $where .= " AND {$a}.user_id=" . $employeeId;
  if ($status !== 'all' && in_array($status, ['pending','approved','needs_changes'], true)) {
    $where .= " AND COALESCE(NULLIF({$a}.status,''),'pending')='" . $mysqli->real_escape_string($status) . "'";
  }
  return $where;
}
