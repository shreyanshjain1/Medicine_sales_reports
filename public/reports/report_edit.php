...
$errors=[]; $fieldErrors=[]; $warnings=[]; $duplicates=[]; $ok=false; $draftSaved=false;
...
$draftRows = fetch_user_report_drafts((int)user()['id'], 5);
if($_SERVER['REQUEST_METHOD']==='POST'){
  csrf_verify();
  $intent = (string)post('submit_intent','save');
  ...
  if ($intent === 'save_draft') {
    $draftId = save_report_draft((int)user()['id'], report_draft_payload_from_request($_POST), 0, $id);
    if ($draftId > 0) {
      $draftSaved = true;
      log_audit('report_edit_draft_saved', 'report_draft', $draftId, 'Draft saved from report edit');
      $draftRows = fetch_user_report_drafts((int)user()['id'], 5);
    } else {
      $errors[] = 'Failed to save draft copy.';
    }
  } else {
...
<div class="card">
  ...
  <?php if($draftRows): ?><div class="draft-strip compact"><div class="section-title">Recent Drafts</div><div class="draft-chip-wrap"><?php foreach($draftRows as $draft): ?><a class="draft-chip" href="<?= e(url('reports/report_add.php?draft_id='.(int)$draft['id'])) ?>"><strong><?= e($draft['doctor_name'] ?: 'Untitled Draft') ?></strong><span><?= e(date('M d, Y H:i', strtotime((string)$draft['updated_at']))) ?></span></a><?php endforeach; ?></div></div><?php endif; ?>
