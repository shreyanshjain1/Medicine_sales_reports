...
function hospitals_master_rows(string $where): array {
  global $mysqli; $rows=[];
  if($res=$mysqli->query("SELECT * FROM hospitals_master {$where} ORDER BY name ASC LIMIT 500")) { while($row=$res->fetch_assoc()) $rows[]=$row; $res->free(); }
  return $rows;
}
function hospitals_master_count(string $where): int {
  global $mysqli;
  if($res=$mysqli->query("SELECT COUNT(*) c FROM hospitals_master {$where}")) { $row=$res->fetch_assoc(); $res->free(); return (int)($row['c'] ?? 0); }
  return 0;
}

$where='WHERE active=1';
if($q!==''){ $safe=$mysqli->real_escape_string('%'.$q.'%'); $where .= " AND (name LIKE '{$safe}' OR city LIKE '{$safe}' OR address LIKE '{$safe}' OR notes LIKE '{$safe}')"; }

if(getv('template','')==='csv'){
  csv_download('hospitals-master-template.csv',['Name','City','Address','Notes'],[
    ['Makati Medical Center','Makati City','2 Amorsolo St, Legazpi Village','Key metro hospital'],
    ['St. Luke\'s Medical Center','Quezon City','279 E Rodriguez Sr. Ave','West wing access contact if needed'],
  ]);
}
  ...
  if($action==='delete'){
    ...
    if(!$importRows) {
      ...
      foreach($importRows as $row){
        $name=trim((string)($row['name'] ?? $row['hospital'] ?? $row['hospital name'] ?? ''));
        ...
        if(master_find_possible_duplicates('hospitals_master','name',$name,0,['city'=>$city,'address'=>$address])){ $importSummary['duplicate_skipped']++; if(count($importSummary['samples']) < 5) $importSummary['samples'][]=$sample; continue; }
        $stmt=$mysqli->prepare('INSERT INTO hospitals_master (name,city,address,notes,active) VALUES (?,?,?,?,1)');
        if($stmt){ $stmt->bind_param('ssss',$name,$city,$address,$notes); if($stmt->execute()) $importSummary['added']++; $stmt->close(); }
      }
    ...
    if(!$errors && $duplicateRows && !$forceSave) $warnings[]='Possible duplicate hospitals or clinics were found. Review them below before saving, or click Save Anyway only if this should stay as a separate record.';
...
$totalActive = hospitals_master_count('WHERE active=1');
$rows=hospitals_master_rows($where);
$filteredCount=count($rows);
$searchState = $q !== '' ? 'Search: "'.$q.'"' : 'Showing all active hospitals and clinics';
include __DIR__.'/../header.php';
?>
<div class="crm-hero"><div><h2>Hospitals / Clinics Master</h2><div class="subtle">Keep facility records clean for scheduling, reporting, and export filtering.</div></div></div>
<div class="summary-strip summary-strip--masters">
  <div class="summary-chip"><span class="label">Active facilities</span><strong><?= (int)$totalActive ?></strong></div>
...
<div class="grid two-panels masters-grid">
  <div class="card">
    ...
    <?php if($duplicateRows): ?><div class="alert warning form-alert duplicate-warning-box"><strong>Possible duplicates found</strong><div class="mini-note">Use <strong>Save Anyway</strong> only when this really is a separate facility record.</div><ul class="duplicate-list"><?php foreach($duplicateRows as $dup): ?><li><?= e(($dup['name'] ?? '').' • '.($dup['city'] ?? '').' • '.($dup['address'] ?? '')) ?></li><?php endforeach; ?></ul></div><?php endif; ?>
  ...
  <div class="card">
    ...
    <?php if(!$rows): ?>
      <div class="empty-state empty-state--masters"><h4>No hospitals or clinics found</h4><p><?= $q !== '' ? 'Try a different search or reset the filter.' : 'Start by adding a facility manually or importing a CSV template.' ?></p></div>
    <?php else: ?>
