...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$errors) {
    $sql = "UPDATE events SET title=?, city=?, doctor_id=?, purpose=?, medicine_name=?, hospital_name=?, visit_datetime=?, summary=?, remarks=?, status=?, recurrence_pattern=?, recurrence_until=?, start=?, end=?, all_day=?
    ...
    if (!$stmt) {
      ...
      $recurrence_until_param = ($recurrence_until === '' ? null : $recurrence_until);
      if ($role === 'district_manager') {
        $stmt->bind_param('ssisssssssssssiiii', $title, $city, $doctor_id, $purpose, $medicine_name, $hospital_name, $visit_datetime, $summary, $remarks, $status, $recurrence_pattern, $recurrence_until_param, $start, $end, $all, $id, $uid, $uid);
      } else {
        $stmt->bind_param('ssisssssssssssiii', $title, $city, $doctor_id, $purpose, $medicine_name, $hospital_name, $visit_datetime, $summary, $remarks, $status, $recurrence_pattern, $recurrence_until_param, $start, $end, $all, $id, $uid);
      }
      if ($stmt->execute()) {
        $stmt->close();
  ...
  // refresh
  $stmt = $mysqli->prepare("SELECT e.*, d.dr_name, d.speciality, d.hospital_address, d.place
   FROM events e LEFT JOIN doctors_masterlist d ON d.id = e.doctor_id WHERE {$where} LIMIT 1");
  if ($role === 'district_manager' && $hasEA)      $stmt->bind_param('iiii', $id, $uid, $uid, $uid);
  elseif ($role === 'district_manager' && !$hasEA) $stmt->bind_param('iii', $id, $uid, $uid);
  elseif ($role !== 'manager' && $hasEA)           $stmt->bind_param('iii', $id, $uid, $uid);
  else                                              $stmt->bind_param('ii', $id, $uid);
...
$selectedAtt = [];
if ($role === 'manager') {
  if ($r=$mysqli->query("SELECT id,name,role FROM users WHERE active=1 ORDER BY role ASC, name ASC")) while($u=$r->fetch_assoc()) $repUsers[]=$u;
  ...
  if ($stmtU) {
    $stmtU->bind_param('ii',$uid,$uid); $stmtU->execute(); $rsU=$stmtU->get_result();
    while($u=$rsU->fetch_assoc()) $repUsers[]=$u;
    $stmtU->close();
  }
}
if (!empty($repUsers) && $hasEA) {
  $stmtS = $mysqli->prepare("SELECT user_id FROM event_attendees WHERE event_id=?");
  if ($stmtS) {
    $eid=(int)$ev['id']; $stmtS->bind_param('i',$eid); $stmtS->execute(); $rsS=$stmtS->get_result();
    ...
    <div class="grid two">
      ...
      <label class="form-field titlecase"><span class="form-label">City <span class="req">*</span></span>
        <select class="<?= field_class($fieldErrors, 'city') ?>" name="city" id="edit_city" required>
          ...
          <?php foreach($cities as $c): ?><option value="<?= e($c) ?>" <?= ($c === ($ev['city'] ?? '')) ? 'selected' : '' ?>><?= e($c) ?></option><?php endforeach; ?>
        </select><?= field_error_html($fieldErrors, 'city') ?>
      </label>
      <label class="form-field titlecase"><span class="form-label">Doctor</span>
        <select class="form-control" name="doctor_id" id="edit_doctor"><option value="">Select Doctor</option></select>
      ...
      <label class="form-field titlecase"><span class="form-label">Status</span>
        <select class="form-control" name="status">
          <?php foreach ($statusOptions as $k => $label): ?><option value="<?= e($k) ?>" <?= (($ev['status'] ?? 'planned') === $k) ? 'selected' : '' ?>><?= e($label) ?></option><?php endforeach; ?>
        </select>
      </label>
      <label class="form-field titlecase"><span class="form-label">Repeats</span>
        <select class="form-control" name="recurrence_pattern">
          <?php foreach ($recurrenceOptions as $k => $label): ?><option value="<?= e($k) ?>" <?= (($ev['recurrence_pattern'] ?? 'none') === $k) ? 'selected' : '' ?>><?= e($label) ?></option><?php endforeach; ?>
        </select>
      </label>
      <?php render_text_input('Repeat Until', 'recurrence_until', (string)($ev['recurrence_until'] ?? ''), ['type'=>'date'], $fieldErrors); ?>
      <label class="chk form-check titlecase" style="align-self:end"><input type="checkbox" name="all_day" value="1" <?= !empty($ev['all_day']) ? 'checked' : '' ?>> All Day</label>
      ...
      <label class="form-field titlecase" style="grid-column:1/-1"><span class="form-label">Reps attending (optional)</span>
        <select class="form-control" name="attendees[]" multiple size="6">
          <?php foreach($repUsers as $u): if ((int)$u['id'] === (int)$uid) continue; ?>
  ...
  async function loadDoctors(city){
    ...
    try{
      ...
      list.forEach(d=>{
        ...
        o.value = d.id; o.textContent = (d.dr_name || 'Doctor') + (d.speciality ? ' — ' + d.speciality : ''); docSel.appendChild(o);
      });
