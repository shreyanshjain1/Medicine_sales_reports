  ...
  <?php if ($hasEA): ?>
    <?php
      ...
      if ($stmtA) {
        ...
        while ($rA = $rsA->fetch_assoc()) {
          $attNames[] = $rA['name'] . ' (' . $rA['role'] . ')';
        }
        $stmtA->close();
      }
    ?>
    <?php if (!empty($attNames)): ?>
      <p style="margin-top:10px"><strong>Other reps attending:</strong> <?= e(implode(', ', $attNames)) ?></p>
    <?php endif; ?>
  <?php endif; ?>
  <hr>
  <div style="display:flex;gap:10px;flex-wrap:wrap">
    <a class="btn primary" href="<?= url('report_add.php?prefill=1&event_id='.(int)$ev['id'].'&doctor_id='.(int)$ev['doctor_id']) ?>">Generate Report From Task</a>
    <a class="btn" href="<?= url('task_edit.php?id='.(int)$ev['id']) ?>">Edit Task</a>
    <form method="post" action="<?= url('task_delete.php') ?>" style="margin:0" onsubmit="return confirm('Delete this task?');">
      <?php csrf_input(); ?>
